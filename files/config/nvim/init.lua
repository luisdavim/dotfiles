...
-- safely('later', function() require('mini.animate').setup() end)
-- safely('later', function() require('mini.base16').setup() end)
-- safely('later', function() require('mini.colors').setup() end)
-- safely('later', function() require('mini.hues').setup() end)
...
safely("now", function()
	...
	mason_lspconfig.setup({
		...
		ensure_installed = { "lua_ls", "gopls", "bashls", "jsonls", "yamlls", "typos-lsp" },
	})
	...
	vim.api.nvim_create_autocmd("LspAttach", {
		...
		callback = function(ev)
			...
			end, { buffer = ev.buf, desc = "Code actions" })
			...
			end, { buffer = ev.buf, desc = "go to next diagnostic" })
			keymap("n", "[g", function()
				vim.diagnostic.jump({ count = -1, float = true })
			end, { buffer = ev.buf, desc = "go to previous diagnostic" })
